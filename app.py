...
def clean_and_process_data(df):
    ...
    for col in df_clean.columns:
        ...
        elif 'Correo electr' in col_clean:
            column_mapping[col] = 'Correo'
        elif 'Rol o relaci' in col_clean:
            column_mapping[col] = 'Rol'
        elif 'rea o proceso' in col_clean:
        ...
        elif 'Cu' in col_clean and 'l es tu propuesta' in col_clean:
        ...
        elif 'Tiempo de implementaci' in col_clean:
    ...
    text_columns = ['Nombre_Colaborador', 'Area', 'Nombre_Iniciativa', 'Problema', 
                    'Propuesta', 'Beneficios', 'Proceso_Relacionado']
    ...
    df_clean['Puntuacion_Ponderada'] = (
        df_clean['Valor_Estrategico'] * 0.20 +
        df_clean['Nivel_Impacto'] * 0.20 +
        df_clean['Viabilidad_Tecnica'] * 0.15 +
        df_clean['Costo_Beneficio'] * 0.15 +
        df_clean['Innovacion_Disrupcion'] * 0.10 +
        df_clean['Escalabilidad_Transversalidad'] * 0.10 +
        df_clean['Tiempo_Implementacion'] * 0.10
    )
...
def generate_pdf_report(df_filtered):
    ...
    title_style = ParagraphStyle(
        ...
        alignment=1, textColor=colors.HexColor('#2d5aa0')
    )
    heading_style = ParagraphStyle(
        'CustomHeading', parent=styles['Heading2'], fontSize=16, spaceAfter=12,
        spaceBefore=20, textColor=colors.HexColor('#1f4e79')
    )
    normal_style = ParagraphStyle(
        'CustomNormal', parent=styles['Normal'], fontSize=11, spaceAfter=6, alignment=0
    )
    ...
    with st.form(key="login_form"):
        ...
        if submit_button:
            if check_credentials(username, password):
                ...
                st.success(f"✅ Inicio de sesión exitoso para {username}")
                st.rerun()
            else:
...
def main():
    ...
    if (st.session_state["authenticated"] and st.session_state["last_activity"] and 
        (datetime.now() - st.session_state["last_activity"]).total_seconds() > 1800):
        st.session_state["authenticated"] = False
        st.session_state["username"] = None
        st.session_state["last_activity"] = None
    ...
    # Update last activity timestamp on every interaction if authenticated
    ...
    <div class="main-header">
        ...
        <p>Sistema de Análisis y Priorización de Propuestas | Bienvenido, {st.session_state["username"]}</p>
    </div>
    ''', unsafe_allow_html=True)
